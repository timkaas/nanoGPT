...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        if self.flash:
            ...
            mm = attn_mask.float().masked_fill(attn_mask.logical_not(), float('-inf'))
            attn_mask2 = mm.unsqueeze(1).repeat(1, self.n_head, 1, 1)  # attn_mask : [batch_size x n_heads x len_q x len_k]
            att_w = torch.softmax((q @ k.transpose(-2, -1) / math.sqrt(q.size(-1))) + attn_mask2, dim=-1)
